...
function my_loading_show(){
	...
	my_loading_index = layer.load(0, {shade:[0.1, '#000']});
}
...
function my_ajax_json(url, data, success_fun, error_fun) {
	$.ajax({
		...
		error : error_fun ? error_fun : function(XMLHttpRequest, textStatus, errorThrown) {}
	});
}
function my_ajax_json_get(url, data, success_fun, error_fun) {
	$.ajax({
		type : "GET",
		url : url,
		data : data,
...
//ajax方式提交--直接传数据
function ajax_submit_data(url, post_data, callback){
	my_ajax_json(url, post_data, function (data){
...
function log(){
	...
	try{
		...
		if(Browser.Firefox){
			console.log.apply(console, [].concat(get_date('time'), argsArray));
		}else{
	...
	//console.log(getErrorStackTrace());
	//var argsArray = Array.prototype.slice.call(arguments);
	//console.log.apply(console, arguments);

	//arguments.push(console.trace());
	//console.log.apply(console, arguments);
...
function getErrorStackTrace() {
	var obj = {};
	Error.captureStackTrace(obj, getErrorStackTrace);
	return obj.stack;
}
